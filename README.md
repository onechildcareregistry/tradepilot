...
Each approved plan may contain one to three execution candidates and up to twelve research runner-ups. Runner-ups are tracked for evaluation only and cannot produce orders. Completed one-minute bars remain in SQL for 30 days, then archive as compressed private blobs; blob expiry 60 days after archiving gives approximately 90 days of total retention. Exact streamed prices are retained only when they support a risk decision, order, fill, exit, or position change. This preserves a compact observed price path without retaining a full tick tape. Archival must complete before SQL pruning. Original plans, evidence, financial ledger, and derived outcomes are retained. Monitor the Basic SQL 2 GB limit; the initial design targets at most fifteen research symbols, not a large universe.
